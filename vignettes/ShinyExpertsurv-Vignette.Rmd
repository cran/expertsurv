...
In Figure \@ref(fig:expertsurv-1) we have the following steps to upload the data:
  ...
  * Upload an excel file containing the survival data
  * Select the columns referring to the time, status and arm (if jointly modelling treatment and comparator)
  * Set the limit of the for the survival plot (ensuring that Choose opinion type as ''Survival at timepoint(s)''
  * We assume that there are two experts who we will provide expert opinion 
  * Once the above parameters are defined select the ''Plot/Update Survival Curves and Expert Opinions''
...
In Figure \@ref(fig:expertsurv-2) we have the following steps to elicit the expert's opinions:
  ...
  * We set the timepoint at which we elicit expert opinion to 20 months.
  * Experts will be asked for their beliefs on the median survival at 20 months, the lower $2.5\%$ and upper $97.5\%$ probabilities of the population survival. 
...
In Figure \@ref(fig:expertsurv-2) we have the following steps to run the analysis:
  ...
  * We can select which treatment arm relates to the expert's belief (i.e. treatment or comparator).
  * Selecting Run Analysis will conduct the analysis; and it should be noted that the Bayesian analysis can take a considerable amount of time.
  * Once the analysis is complete we can download the $\texttt{expertsurv}$ object which can be loaded an R session later.
  * We can also download the results as a report in one of three formats (HTML, PDF or Word) as shown in Figure \@ref(fig:expertsurv-4).
