...
minusloglik.flexsurv <- function(optpars, Y, X=0, weights, bhazard, rtrunc, 
    ...
    logLikFactory(Y=Y, X=X, weights=weights, bhazard=bhazard,
                  ...
                  dfns=dfns, aux=aux, mx=mx, fixedpars=fixedpars,expert_opinion = expert_opinion)(optpars)
}
                        ...
                        integ.opts=NULL, sr.control=survreg.control(), hessian=TRUE, hess.control=NULL,expert_opinion = NULL, ...)
{
    ...
    if ((is.logical(fixedpars) && fixedpars==TRUE) ||
        ...
        minusloglik <- minusloglik.flexsurv(inits, Y=Y, X=X,
                                            ...
                                            dfns=dfns, aux=aux, mx=mx, expert_opinion= expert_opinion)
        ...
        minusloglik <- minusloglik.flexsurv(res.t[,"est"], Y=Y, X=X, weights=weights, bhazard=bhazard, rtrunc=rtrunc, 
                                            dlist=dlist, inits=inits, dfns=dfns, aux=aux, mx=mx, expert_opinion = expert_opinion)
...
deriv_supported <- function(Y){
  ...
  return(FALSE)#Never want to use the analytical derivatives as this doesn´t include the expert opinion
}
